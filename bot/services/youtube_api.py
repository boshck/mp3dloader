...
class YouTubeAPI:
    ...
    def __init__(self):
        """Инициализация YouTube API сервиса"""
        ...
        logger.info("YouTube API service initialized (via yt-dlp)")
    
    async def extract_track_info(self, url: str) -> Optional[YouTubeTrack]:
        """
        ...
        Returns:
            YouTubeTrack или None
        """
        try:
            logger.debug(f"Extracting YouTube track info: {url}")
            ...
            track = YouTubeTrack.from_yt_dlp_info(info)
            
            if track:
                logger.info(f"YouTube track extracted: {track.title} by {track.channel}")
            
            return track
            
        except Exception as e:
            logger.error(f"Error extracting YouTube track info: {e}")
            return None
    
    async def extract_playlist_info(self, url: str, max_tracks: int = 50) -> List[YouTubeTrack]:
        """
        Получить список треков из плейлиста (download=False, первые 50 валидных)
        
        Args:
            url: URL YouTube плейлиста
            max_tracks: Максимальное количество валидных треков
            
        Returns:
            Список YouTubeTrack (до max_tracks валидных треков)
        """
        try:
            ...
            if not playlist_info or 'entries' not in playlist_info:
                logger.warning("No entries found in YouTube playlist")
                return []
            ...
            for entry in entries:
                ...
                try:
                    ...
                    if 'duration' not in entry:
                        ...
                        track = await self.extract_track_info(video_url)
                    else:
                        # Уже есть вся информация
                        track = YouTubeTrack.from_yt_dlp_info(entry)
            ...
            logger.info(f"Extracted {len(tracks)} valid tracks from YouTube playlist")
            return tracks
    ...
    async def download_audio(self, track: YouTubeTrack) -> Optional[str]:
        """
        Скачать аудио через yt-dlp
        
        Args:
            track: Объект YouTubeTrack
            
        Returns:
            Путь к скачанному файлу или None
        """
        try:
            logger.info(f"⬇️ Downloading YouTube track: '{track.title}' by '{track.channel}'")
